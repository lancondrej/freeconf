...
class PackageInterface (object):
	...
	def window (self):
		...
		if self._package is None: return
		return self._package.trees.guiTree.proxy

	def showAllActiveWidgets (self, show):
		"""Turns showing advanced widgets on or off"""
		self._package.trees.guiTree.showAll(show)
...
class FcGUICommunicator(object):
	...
	def minWidth (self):
		...
		if isinstance(self.__guiEntry, FcGWindow):
	...
	def icon (self):
		...
		if isinstance(self.__guiEntry, FcGTab):
	...
	def empty (self):
		...
		if isinstance(self.__guiEntry, FcGTab) or isinstance(self.__guiEntry, FcCGSEntry):
	...
	def children (self):
		...
		if isinstance(self.__guiEntry, FcCGSEntry):
	...
	def enabled (self):
		...
		if isinstance(self.__guiEntry, GEntry) and self.__guiEntry.configBuddy != None:
